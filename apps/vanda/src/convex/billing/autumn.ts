...
type BillingCustomer = z.infer<typeof CustomerSchema>;

const AttachResponseSchema = z.object({
  payment_url: z.string().nullable().optional(),
  required_action: z.json().optional(),
});

interface BillingCustomerResult {
  data: BillingCustomer | null;
  error: { code?: string; message?: string } | null;
}

/** Provider boundary exposed as an object so tests can inject a faithful reader. */
export const billingDependencies = {
  getCustomer: async (
    ctx: Parameters<typeof autumn.customers.get>[0],
  ): Promise<BillingCustomerResult> => {
    const result = await autumn.customers.get(ctx);

    return {
      data: result.data === null ? null : CustomerSchema.parse(result.data),
      error: result.error,
    };
  },
};
...
const snapshotOf = (
  customer: { products?: CustomerProduct[] | undefined } | null,
): BillingSnapshot => {
  const products = customer?.products?.filter(
    (product) => product.id && PLAN_PRODUCT_IDS.some((planId) => planId === product.id),
  );
...
export const syncBilling = action({
  ...
  handler: async (ctx): Promise<BillingSnapshot | null> => {
    ...
    const result = await billingDependencies.getCustomer(ctx);
...
export const startCheckout = action({
  ...
  handler: async (ctx, args): Promise<{ checkoutUrl: string | null; attached: boolean }> => {
    ...
    const customer = customerOrNull(await billingDependencies.getCustomer(ctx));
    ...
    for (const product of products) {
      if (
        ...
        PLAN_PRODUCT_IDS.some((planId) => planId === product.id) &&
...
export const previewPlanChange = action({
  ...
  handler: async (ctx, { planId, schedule }) => {
    ...
    if (!identity) throw new Error("Not authenticated");
    const current = snapshotOf(customerOrNull(await billingDependencies.getCustomer(ctx)));
  ...
  handler: async (ctx, args): Promise<{ checkoutUrl: string | null; attached: boolean }> => {
    ...
    try {
      ...
      try {
        ...
        const result = AttachResponseSchema.parse(
          await billingRequest("attach", {
            ...params,
...
export const syncCallerBilling = internalAction({
  ...
  handler: async (ctx, { clerkId }) => {
    ...
    const snapshot = snapshotOf(CustomerSchema.parse(await response.json()));
    await ctx.runMutation(internal.billing.autumn.applySnapshot, {
      clerkId,
      planId: snapshot.planId,
      periodStart: snapshot.periodStart,
