...
export function parseWebhook(
  ...
  const batch = z
    .object({ batch: z.literal(true), data: z.array(z.unknown()).max(100) })
    .safeParse(body);
  const items = batch.success ? batch.data.data : [body];
  return items.flatMap((item) => {
    const parsed = messageSchema.parse(item);
    ...
    return [
      {
        ...
        sender,
        recipientKind: bsuid ? ("bsuid" as const) : ("phone" as const),
        ...(phone && /^\+?\d{7,15}$/.test(phone) ? { phone: phone.replace(/^\+/, "") } : {}),
