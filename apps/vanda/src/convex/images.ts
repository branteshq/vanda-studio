...
async function paintImage(
...
}> {
  {
    ...
    const paintInput: PaintInputArgs = {
      accountId,
      referenceImageIds: referenceImageIds ?? [],
    };

    if (editOfImageId) paintInput.editOfImageId = editOfImageId;
    const resolved = await ctx.runQuery(internal.imagesData.resolvePaintInput, paintInput);
    ...
    try {
      if (sub.active && sub.userId) {
        ...
        if (inputReferences.length > 0) generationArgs.referenceUrls = inputReferences;
        generated = await codexGenerateImage(generationArgs);
      } else {
        ...
        generated = await Effect.runPromise(
          Effect.flatMap(ImageAssetGenerator, (generator) =>
            generator.generate(
              (() => {
                const generationArgs: GenerateImageArgs = {
                  prompt: trimmedPrompt,
                  aspectRatio,
                ...
                if (tier !== "1K") generationArgs.resolution = tier;
    ...
    const savedImage: SavePaintedImageArgs = {
      accountId,
      storageId,
      prompt: trimmedPrompt,
      mimeType,
    ...
    if (name?.trim()) savedImage.name = name.trim();

    if (generated.costUsd !== undefined) savedImage.costUsd = generated.costUsd;

    if (promptAuthor) savedImage.promptAuthor = promptAuthor;

    if (placeholderImageId) savedImage.placeholderId = placeholderImageId;

    if (editOfImageId) savedImage.editOfImageId = editOfImageId;

    if (activityId) savedImage.activityId = activityId;
    const imageId = await ctx.runMutation(internal.imagesData.savePaintedImage, savedImage);
